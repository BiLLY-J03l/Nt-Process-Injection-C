#include <stdio.h>
#include <windows.h>


void encrypt(shellcode,shellcode_size,key)
	unsigned char shellcode[];
	SIZE_T shellcode_size;
	char key;
{
	printf("[+] ENCRYPTING with \'%c\' key\n",key);
	for(int i=0; i<shellcode_size; i++){	
		printf("\\x%02x",shellcode[i]^key);
		shellcode[i] = shellcode[i]^key;
	}
	return;
}

void decrypt(shellcode,shellcode_size,key)
	unsigned char shellcode[];
	SIZE_T shellcode_size;
	char key;
{
	printf("[+] DECRYPTING with \'%c\' key\n",key);
	for (int i=0; i<shellcode_size; i++){
		printf("\\x%02x",shellcode[i]^key);
		shellcode[i]=shellcode[i]^key;	
	}
	return;
}

int main(){
	
	unsigned char shellcode[] = {
		
	};
	SIZE_T shellcode_size=sizeof(shellcode);
	char key1= 'P';
	char key2= 'L';
	char key3= 'S';
	char key4= 'a';
	char key5= '5';
	
	printf("[+] ORIGINAL SHELLCODE\n");
	for(int i=0; i<sizeof(shellcode); i++){	
		printf("\\x%02x",shellcode[i]);
		
	}
	printf("\n");
	encrypt(shellcode,shellcode_size,key1);
	printf("\n");
	encrypt(shellcode,shellcode_size,key2);
	printf("\n");
	encrypt(shellcode,shellcode_size,key3);
	printf("\n");
	encrypt(shellcode,shellcode_size,key4);
	printf("\n");
	encrypt(shellcode,shellcode_size,key5);
	printf("\n");
	decrypt(shellcode,shellcode_size,key5);
	printf("\n");
	decrypt(shellcode,shellcode_size,key4);
	printf("\n");
	decrypt(shellcode,shellcode_size,key3);
	printf("\n");
	decrypt(shellcode,shellcode_size,key2);
	printf("\n");
	decrypt(shellcode,shellcode_size,key1);
	
	printf("\n");

	//printf("\\x%02x",shellcode);
	
	return 0;
}